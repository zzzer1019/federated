...
def get_tf_typespec_and_binding(parameter_type, arg_names):
  ...
  Returns:
    ...
    list and kwargs_typespec is a dict, both containing `tf.TensorSpec` objects.
    These structures are intended to be passed to the `get_concrete_function`
    method of a `tf.function`.
    Note the "binding" is "preliminary" in that it includes the names of the
    TensorSpecs produced; these must be converted to the names of actual tensors
    based on the SignatureDef of the SavedModel before the binding is finalized.
  """
...
def get_tf2_result_dict_and_binding(result, name_prefix=None):
  ...
  Args:
    result: The result returned by a Python TensorFlow function
      (possibly a tf.function), which will be flattened as a dict to allow
      it to be returned from a tf.function which is to be serialized.
      Thus, `result` should be a Python object that is composed of
  ...
  def _handle_elements(name_value_pairs, container_type=None):
    ...
    if container_type:
      type_fn = lambda t: computation_types.NamedTupleTypeWithPyContainerType(
        t, container_type=container_type)
    else:
      type_fn = computation_types.NamedTupleType

    return (type_fn(ntt_pairs),
            pb.TensorFlow.Binding(
                tuple=pb.TensorFlow.NamedTupleBinding(
  ...
  def _get_result_type_and_binding_impl(result, name_prefix):
    ...
    Has the side effect of updating `result_dict` as well.
    """
    if tf.contrib.framework.is_tensor(result):
      ...
      return _handle_elements(
        name_value_pairs, container_type=type(result))
    elif isinstance(result, anonymous_tuple.AnonymousTuple):
      return _handle_elements(anonymous_tuple.to_elements(result))
      ...
      return _handle_elements(name_value_pairs, container_type=type(result))
