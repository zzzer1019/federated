...
def _uniquify_shared_names(graph, to_append):
  """Appends `to_append` to any shared names present in `graph`."""
  ...
  for x in graph_def.node:
    if 'shared_name' in x.attr.keys():
      x.attr['shared_name'].s += tf.compat.as_bytes(to_append)
      del x.attr['shared_name']
...
def _concat_graphs(graph_def_list, graph_names_list):
  ...
  for k in range(len(graph_names_list)):
    # merged_graph must be overwritten here without shared_names to prevent
    # variables being wired together incorrectly.
    merged_graph = _uniquify_shared_names(merged_graph,
                                          str(six.next(name_generator)))
